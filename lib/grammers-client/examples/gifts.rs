//! Example to print the ID of owned gifts
//!
...
const SESSION_FILE: &str = "gifts.session";
...
async fn async_main() -> Result<()> {
    ...
    let mut gifts = client.iter_gifts(
        client.get_me().await?.pack(),
        false,
        false,
        false,
        false,
    ...
    println!("Printing gifts");

    while let Some(gift) = gifts.next().await? {
        println!(
            "Gift ID: {}, Upgradable: {}, Premium Only: {}",
            gift.id(),
            gift.is_upgradable(),
