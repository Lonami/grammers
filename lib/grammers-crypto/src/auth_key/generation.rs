...
impl fmt::Display for AuthKeyGenError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidNonceHash { got, expected } => write!(
                f,
                "invalid nonce hash: got {:?}, expected {:?}",
                got, expected
            ),
        }
    }
...
fn do_step2(
    ...
    if nonce != res_pq.nonce {
        return Err(AuthKeyGenError::InvalidNonce {
    ...
    let ciphertext = rsa_encrypt(&pq_inner_data, n, e, &random_bytes);
...
fn do_step3(
    ...
    if server_dh_params.server_nonce != server_nonce {
        return Err(AuthKeyGenError::InvalidServerNonce {
...
pub fn create_key(data: Step3, response: Vec<u8>) -> Result<(AuthKey, i32), AuthKeyGenError> {
    ...
    if dh_hash != new_nonce_hash {
        return Err(AuthKeyGenError::InvalidNonceHash {
            got: dh_hash,
            expected: new_nonce_hash,
        });
    }
...
fn rsa_encrypt(data: &[u8], n: BigUint, e: BigUint, random_bytes: &[u8; 256]) -> Vec<u8> {
...
mod tests {
    ...
    fn test_rsa_encryption() {
        ...
        let result = rsa_encrypt(b"Hello!", n, e, &[0; 256]);
        assert_eq!(
            result,
            vec![
