...
async fn test_qr_login_functionality() {
    ...
    let user = wait_for_qr_login(&client, api_id, api_hash, std::time::Duration::from_secs(120)).await
        .expect("Failed to complete QR login within timeout");
    
    // Verify successful login
    println!("Successfully logged in as: {:?}", user.first_name().unwrap_or("Unknown"));
    ...
    match client.invoke(&grammers_client::tl::functions::updates::GetState {}).await {
...
async fn wait_for_qr_login(
    ...
    loop {
                ...
                Ok(login_token) => {
                    match login_token {
                        grammers_tl_types::enums::auth::LoginToken::Token(token) => {
                            let new_qr_url = format!("tg://login?token={}", 
                                base64::engine::general_purpose::URL_SAFE_NO_PAD.encode(&token.token)
                            );
                        ...
                        grammers_tl_types::enums::auth::LoginToken::MigrateTo(migrate_to) => {
                            ...
                            match client.import_login_token(migrate_to.token, migrate_to.dc_id).await {
                                Ok(import_result) => {
                                    match import_result {
                                        grammers_tl_types::enums::auth::LoginToken::Success(success) => {
                                ...
                                Err(grammers_mtsender::InvocationError::Rpc(ref err)) if err.name == "SESSION_PASSWORD_NEEDED" => {
                                    // Handle 2FA required case after migration
                                    println!("2FA password required after migration. Getting password information...");
                                            ...
                                            match std::env::var("TG_2FA_PASSWORD") {
                                                Ok(password) => {
                                                    ...
                                                    match client.check_password(password_token, password.as_bytes()).await {
                                                        Ok(user) => {
                                                            println!("Successfully logged in with 2FA after migration");
                                                            return Ok(user);
                                                        },
                                                        Err(sign_in_error) => {
                                                            match sign_in_error {
                                                                grammers_client::SignInError::Other(invocation_error) => {
                                                                    println!("Failed to complete login with 2FA after migration: {}", invocation_error);
                                                                    return Err(Box::new(invocation_error));
                                                                },
                                                                _ => {
                                                                    println!("Failed to complete login with 2FA after migration: {}", sign_in_error);
                                                ...
                                                Err(_) => {
                                                    println!("TG_2FA_PASSWORD environment variable not set. Please set it to complete 2FA login.");
                                                    return Err(Box::new(grammers_mtsender::InvocationError::Rpc(grammers_mtsender::RpcError {
                                                        code: 401,
                                ...
                                Ok(password) => {
                                    ...
                                    match client.check_password(password_token, password.as_bytes()).await {
                                        ...
                                        Err(sign_in_error) => {
                                            match sign_in_error {
                                                grammers_client::SignInError::Other(invocation_error) => {
                                                    println!("Failed to complete login with 2FA: {}", invocation_error);
                                                    return Err(Box::new(invocation_error));
                                                },
                                                _ => {
                                                    println!("Failed to complete login with 2FA: {}", sign_in_error);
                                                    return Err(Box::new(sign_in_error));
                                                }
                                            }
                                        }
        ...
        if loop_count % 5 == 0 {  // Print every 5 seconds
